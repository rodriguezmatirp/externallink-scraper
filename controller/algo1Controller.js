...
module.exports.algo1 = async (req) => {
    ...
    if (find.length > 0) {
        ...
        response = await new Promise((resolve, reject) => {
            parser.parseString(html, async function (err, result) {
                const doc = await algo1insertSiteMap(result, url, result['sitemapindex']['sitemap'].length)
                if (doc) {
...
const algo1insertSiteMap = async (result, url, length) => {
    try {
        ...
        var arr = []
        for (i = 0; i < length; i++) {
            arr.push({ "link": (result['sitemapindex']['sitemap'][i].loc[0]), "page": i + 1 });
